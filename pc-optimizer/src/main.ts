...
/** Separado da busca: a abertura e o botão de analisar de novo usam o mesmo resultado. */
 ...
 * quando existe. Passando de quatro, o resto abre ali mesmo.
 */
function mostrarOutrosProblemas(v: Veredito) {
  ...
  const itens = outros
    .map((a, i) => {
      const acao = a.acao
        ...
        : "";
      return `<li${i >= OUTROS_NO_PAINEL ? " hidden" : ""}><strong>${escapeHtml(a.title)}</strong> — ${escapeHtml(a.measured)}${acao}</li>`;
    })
    .join("");
  const resto = outros.length - OUTROS_NO_PAINEL;
  const mais = resto > 0
    ? `<button class="btn btn-ghost btn-small veredito-outros-mais" type="button" data-outros-todos>${resto === 1 ? "Ver o outro" : `Ver os outros ${resto}`}</button>`
    : "";
  ...
  bloco.querySelectorAll<HTMLButtonElement>("[data-outro]").forEach((botao) => {
    const acao = outros[Number(botao.dataset.outro)]?.acao;
  ...
  bloco.querySelector<HTMLButtonElement>("[data-outros-todos]")?.addEventListener("click", (e) => {
    bloco.querySelectorAll<HTMLElement>("li[hidden]").forEach((li) => (li.hidden = false));
...
function fraseDaQueda(q: Deriva): string {
  ...
  switch (q.mudou.tipo) {
    ...
    case "Nada":
      return `${numeros}, sem mudança de driver nem de Windows. Vale olhar a temperatura e o que está rodando junto com o jogo; o Início aponta os dois quando o Windows registra.`;
  }
}
...
function wireControls() {
  ...
  element("analyze-rbar").addEventListener("click", analyzeRbar);  element("analyze-fivem").addEventListener("click", analyzeFiveM);
