// Saúde do hardware: disco morrendo é o "otimizei e não melhorou" que nenhum ajuste conserta, e bateria gasta
// faz o Windows limitar o processador.
...
/// Quatro casos: "perguntei e não há disco" e "não consegui perguntar" viravam a mesma lista vazia, e a tela
/// inventava a causa (máquina virtual) de uma leitura que nunca aconteceu.
...
/// `None` = não consegui perguntar; `Some(0)` = perguntei e não há.
...
/// Os valores vêm em inglês em qualquer idioma.
...
/// Três respostas: `unwrap_or(0)` dava atestado de saúde a um SSD que não publica o contador.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub fn avaliar_erros(leitura: Option<u64>, gravacao: Option<u64>) -> ErrosDoDisco {
    // Um lado ausente impede o total: somar com zero inventaria a metade que falta.
...
/// Extraída para a costura entre `avaliar_erros` e o achado ser testável sem hardware.
fn achados_de_confiabilidade(contador: &RawReliability) -> Vec<HealthFinding> {
    ...
    match avaliar_erros(contador.read_errors_total, contador.write_errors_total) {
        ...
        ErrosDoDisco::Nenhum => {}
        // Este SSD não publica ReadErrorsTotal/WriteErrorsTotal (comum): o achado avisa a lacuna.
        ErrosDoDisco::NaoSei => {
            findings.push(HealthFinding {
    ...
    // Acima de 60 °C um SSD reduz a própria velocidade.
...
fn analisar_bateria(findings: &mut Vec<HealthFinding>) {
    // Sem bateria é desktop: não há o que avaliar.
...
mod tests {
    ...
    fn disco_em_aviso_manda_fazer_backup() {
        // A mensagem manda salvar os arquivos, não otimizar.
    ...
    fn contador_sem_leitura_de_erro_gera_o_achado_naosei_e_nao_o_critico() {
        // A costura: se alguém voltar a `unwrap_or(0) + unwrap_or(0)`, este teste falha.
